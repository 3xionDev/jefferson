...
for (let i = 0; i < (render.options.width > 600 ? 30 : 15); i++) {
    ...
    //     }
    // });
    particles[i] = Bodies.polygon((Math.random() * render.options.width), (Math.random() * render.options.height), getRandomInt(3, 8), getRandomArbitrary(50, 90), {
        collisionFilter: {
            mask: 0x0001,
            category: 0x0001
        },
        render: {
            fillStyle: "rgba(119, 119, 255, 0.06)",
            strokeStyle: "rgba(119, 119, 255, 0.3)",
            lineWidth: 0.5
        }
    });
    ...
    Body.setAngularSpeed(particles[i], getRandomArbitrary(-0.01, 0.01));
}
...
setInterval(() => {
    avg = 0;
    particles.forEach((p) => {
        if (Math.abs(Body.getAngularSpeed(p)) < 0.01) {
            Body.setAngularSpeed(p, getRandomArbitrary(-0.05, 0.05))
        }
        Body.setVelocity(p, {x: getRandomArbitrary(-1, 1), y: getRandomArbitrary(-1, 1)});
    })
}, 5000)
...
setInterval(() => {
    ...
    lines = [];
    particles.forEach((p) => {
        if (distance2D(p.position.x, p.position.y, mx, my) < 200) {
            lines.push(Constraint.create({
                ...
                render: {
                    type: "line",
                    anchors: false,
                    strokeStyle: "rgba(119, 119, 255, 0.26)"
                }
            }))
        }
    })
